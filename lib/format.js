...
FormatHelpers.prototype.getHeader = function (entryPoint) {
  let headerStr = colors.green('DADI CLI')

  if (entryPoint) {
    headerStr += colors.white(` (${entryPoint.name})`)
  }
...
FormatHelpers.prototype.getEntryPointCommands = function (entryPoint) {
  const rows = Object.keys(entryPoint.commands)
    .map(command => {
      return {
        key: colors.bold(command),
...
FormatHelpers.prototype.getGeneralHelp = function (entryPoints, invalidCommand) {
  ...
  Object.keys(entryPoints).forEach(entryPoint => {
    lines.push('')
    lines.push(
      this.getEntryPointCommands(entryPoints[entryPoint])
    )
  })

  lines.push('Type \'dadi help <command>\' to learn more about a specific command.')
  lines.push('')
