...
FormatHelpers.prototype.getCommandHelp = function (entryPoint, command) {
  ...
  lines.push('')
  lines.push(`$ ${colors.bold(`${constants.rootCommand} ${entryPoint.id} ${command}${parametersCommandStr}`)}`)
  lines.push('')
...
FormatHelpers.prototype.getEntryPointHelp = function (entryPoint, invalidCommand) {
  ...
  lines.push('')
  lines.push(`$ ${colors.bold(`${constants.rootCommand} ${entryPoint.id} <command>`)}`)
...
FormatHelpers.prototype.getGeneralHelp = function (entryPoints, invalidCommand) {
  ...
  Object.keys(entryPoints).forEach(entryPoint => {
    ...
    lines.push(
      this.getEntryPointCommands(entryPoints[entryPoint], `${constants.rootCommand} ${entryPoint} `)
    )
  })
