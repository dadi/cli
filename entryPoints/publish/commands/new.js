...
function install ({
  ...
  return registry.downloadBoilerplate({
    ...
    return shell.run(
      shell.command(
        'npm install',
        `Installing DADI Publish (${version})`,
        directoryFullPath
      )
    ).then(out => {
      return `\nAll done! Run the following command to launch your new instance of DADI Publish:\n\n${colors.bold(launchCommand)}\n`
    })
  })
}
...
module.exports = args => {
  ...
  return fsHelpers.warnIfDirectoryIsNotEmpty({
    ...
    const versionMessage = shell.showSpinner('Checking the available versions of DADI Publish')

    return registry.getBoilerplateVersions('publish').then(versions => {
...
module.exports.description = 'Creates a new instance of DADI Publish'
module.exports.parameters = {
  inline: [
    {
      key: 'name',
      description: 'the name of the DADI Publish instance'
    }
  ],
  flags: [
    {
    ...
    {
      key: 'version',
      description: 'the version of Publish to install (defaults to latest)'
    }
  ]
}
